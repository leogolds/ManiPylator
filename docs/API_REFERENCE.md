...
This document covers the public Python API for ManiPylator. All classes below
are importable from the top-level `manipylator` package unless noted otherwise.

---

## Device Classes (`manipylator/devices.py`)

These are the current, recommended classes. Legacy classes (`Robot`,
`SimulatedRobot`, `HeadlessSimulatedRobot`) in `base.py` are deprecated.

### RobotDevice(MQClient)

Base class for all robot devices. Combines RTB kinematics with MQTT device
lifecycle.
...
Adds a Genesis simulation scene (see `KinematicSimulator` vs `PhysicsSimulator` in `base.py`).

Constructor options include `kinematic_physics` (kinematic vs full physics), optional `World`
(obstacle morphs), and `include_ground_plane`. `headless` only toggles the viewer window.
...
Subscribes to `manipylator/robots/+/state` and `manipylator/state`, mirrors
joint angles into a Genesis scene. Useful for real-time visualization of a
physical robot.
...
See also: [MQTT Topics](MQTT_TOPICS.md) for the full topic namespace and message schemas.
